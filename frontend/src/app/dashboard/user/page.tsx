...
import FileInput from "@/components/other/FileInput";
      ...
      <div className="grid grid-cols-1 md:grid-cols-[19rem_1fr] items-start gap-5 mt-5">
        <div className="flex flex-col items-center justify-center p-5 pt-18 overflow-hidden bg-white border border-stone-200 rounded-xl">
          <div className="relative size-20 rounded-full mb-2.5 flex items-center justify-center">
            ...
            <div className="absolute size-30 bg-stone-200/45 rounded-full"></div>
            <div className="absolute size-40 bg-stone-200/35 rounded-full"></div>
            <div className="absolute size-50 bg-stone-200/25 rounded-full"></div>
            <div className="absolute size-60 bg-stone-200/15 rounded-full"></div>
            <Image
              src={fakeUser.avatar}
              alt="user-avatar"
              fill
          ...
          <h3 className="text-lg text-stone-700 font-medium z-20">
            {fakeUser.name}
          </h3>

          {fakeUser.role === "user" && (
            <span className="text-sm">(حساب معمولی)</span>
          )}
          {fakeUser.role === "vendor" && (
            <span className="text-sm">(حساب تولیدی)</span>
          )}
          {fakeUser.role === "admin" && (
            <span className="text-sm">(حساب مدیریت)</span>
          )}

          <LinkButton customClass="mt-2.5 w-full z-20">
            تغییر رمز عبور
          </LinkButton>
          <div className="w-full flex items-center gap-2 mt-2.5 *:grow *:flex-col *:gap-0">
            <LinkButton variation="btn-dim">
              ...
              <span className="text-stone-500">{fakeUser.queriesCount}</span>
            </LinkButton>
            <LinkButton variation="btn-dim">
              <span className="*:size-6">
                <HiMiniHeart />
              </span>
              <span className="text-stone-500">{fakeUser.favoritesCount}</span>
            </LinkButton>
            <LinkButton variation="btn-dim">
              <span className="*:size-6">
                <HiMiniBellAlert />
        ...
        <div className="bg-white border p-5 border-stone-200 rounded-xl">
          <p className="flex items-center gap-1 mb-2.5 p-1 bg-stone-100 text-stone-600 text-sm rounded-lg">
            <span className="*:size-5 text-amber-400">
              <HiLightBulb />
            </span>
            <span>
              کاربر گرامی برای تغییر اطلاعات آنها را با دکمه 'ذخیره اطلاعات' ثبت
              کنید.
            </span>
          </p>
